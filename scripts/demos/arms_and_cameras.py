# CODE DOES NOT WORK: Robot arms seem to be structured differently from the Anymal asset, so there are integration issues with the existing code from "cameras.py" (some sort of deepcopying issue?). More work necessary.
...
class SensorsSceneCfg(InteractiveSceneCfg):
    ...
    # robot
    safe_franka_cfg = copy.deepcopy(FRANKA_PANDA_CFG)
    robot: ArticulationCfg = Articulation(cfg=FRANKA_PANDA_CFG.replace(prim_path="/World/Origin1/Robot"))

    # sensors
    camera = CameraCfg(
        prim_path="/World/Origin1/Robot/base/front_cam",
        update_period=0.1,
        height=480,
        width=640,
    ...
    #     prim_path="/World/Origin1/Robot/base",
