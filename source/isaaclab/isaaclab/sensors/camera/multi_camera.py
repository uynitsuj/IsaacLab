...
class MultiCamera(SensorBase):
    ...
    def __init__(self, cfg: CameraCfg):
        ...
        if self.cfg.spawn is not None:
            ...
            # spawn the asset
            for i in range(self.cfg.cams_per_env):
                self.cfg.spawn.func(f"{self.cfg.prim_path}{i}", self.cfg.spawn, translation=self.cfg.offset.pos, orientation=rot_offset)
        # check that spawn was successful
        
        for i in range(self.cfg.cams_per_env):
            matching_prims = sim_utils.find_matching_prims(f"{self.cfg.prim_path}{i}")
            if len(matching_prims) == 0:
                raise RuntimeError(f"Could not find prim with path {self.cfg.prim_path}{i}.")
    ...
    def set_world_poses(
        ...
        # env_ids: Sequence[int] | None = None, # MUST provide batch of poses for all envs and all cams
        ...
        # if env_ids is None:
        env_ids = self._ALL_INDICES
        assert positions.shape[0] == self.num_instances
